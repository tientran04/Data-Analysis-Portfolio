...
-- 1. INTRODUCTION
-- Crimes have become a problem in modern society since there are more and more crimes nowadays.
-- In this project, I will apply SQL skills to analyze a crime dataset to understand the behaviors of criminals.

-- 2. DATASET
-- The dataset used in this project is the Crime Dataset of the City of Los Angeles from 2020 until the present.
-- Data source: https://catalog.data.gov/dataset/crime-data-from-2020-to-present

-- 3. DATA PREPARATION
-- 3.1. DATA IMPORT
-- Since the data is stored in a CSV file, I will create a table in SQL and import the data from the CSV file.
-- Below are scripts that I use to create the table and import data.
-- Create table
...
-- Compared to 2020, the total cases in 2021 increased by 4.94% and the total cases in 2022 increased by 16.77%.
...
-- The cases fluctuate over months, so there is no correlation between months and the number of cases that happened.
...
-- Friday is the day of the week that has the most cases, followed by Saturday.
...
-- Criminals tend to commit crimes from noon until midnight.
...
-- In 2022, Central is the area that has the most frequent crime with 7.56% of total cases, followed by 77th Street, Southwest, Pacific, and Hollywood areas.
...
-- In 2022, Vehicle Stolen is the most frequent crime type, accounting for 10.61% of total cases, followed by Theft of Identity, Battery - Simple Assault, Burglary From Vehicle, and Burglary.


-- 4.7. VICTIM DISTRIBUTION BY AGE GROUP IN 2022
/* I will classify age into the following groups for this analysis.
...
-- In 2022, Young Adults are the most targeted victims for criminals with 43.92%, followed by Middle-aged Adults at 35.68% and Older Adults at 16.89%.
...
- Friday and Saturday are the two days with the most crimes.
- Criminals tend to commit crimes from noon until midnight.
- In 2022:
	Central, 77th Street, Southwest, Pacific, and Hollywood are the top five areas having the most cases, accounting for almost 30% of total cases.
	Vehicle Stolen and Theft of Identity are the two most happened crime types, accounting for nearly 20% of total cases.
	Young Adults (age from 18 to 35) and Middle-aged Adults (age from 36 to 55) are the two most target victims, accounting for around 80% of total cases.
*/
